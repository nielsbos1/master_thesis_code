...
class MinHashLSH(object):
        ...
        for hash_table in self.hashtables:
            for H in hash_table:
                if len(hash_table[H]) > 1:
                    ...
                    # pairs = {(x[0], x[1]) if x[0][0] < x[1][0] else (x[1], x[0])  for x in pairs }
    ...
    def plot_probability_function(self, step=0.001):
        # define function
        _probability = lambda s : 1 - (1 - s**float(self.r))**float(self.b)
        ...
        ax.set_title(f'threshold = {self.threshold}, not amplified')
